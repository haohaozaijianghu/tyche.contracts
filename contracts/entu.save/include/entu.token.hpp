...
#define ISSUE(bank, to, quantity, memo) \
    {	xtoken::issue_action act{ bank, { {_self, active_perm} } };\
			act.send( to, quantity, memo );}

#define BURN(bank, from, quantity) \
    {	xtoken::burn_action act{ bank, { {_self, active_perm} } };\
...
namespace eosio {
   ...
   /**
    * entu.token contract defines the structures and actions that allow users to create, issue, and manage
   ...
   class [[eosio::contract("entu.token")]] xtoken : public contract {
      public:
         ...
         using burn_action = eosio::action_wrapper<"burn"_n, &xtoken::burn>;
         ...
         using transferx_action = eosio::action_wrapper<"transferx"_n, &xtoken::transferx>;
