...
void dex_burnpool::setsympair( const symbol& token_symbol, const name& token_bank, const asset& base_fgt_amount ) {
    require_auth(_self);
    CHECKC(base_fgt_amount.symbol == ENTU_SYMBOL ,err::PARAM_ERROR, "base_fgt_amount symbol error");

    symbol_pair_t sympair(token_symbol, token_bank);
    CHECKC(!_dbc.get( sympair ),err::RECORD_FOUND,"token symbol is existed");
    
    sympair.base_fgt_amount = base_fgt_amount;
...
void dex_burnpool::opensympair(const name& sympair_code, const bool& on_off) {
    require_auth(_self);
...
void dex_burnpool::ontransfer(const name& from, const name& to, const asset& quant, const string& memo) {
    ...
    // CHECKC(bank == AMAX_TOKEN, err::PARAM_ERROR,  "Invalid FFT bank: " + bank.to_string() )
    _on_receive_fft(from, quant, symbol_from_string(memo));
}

void dex_burnpool::_on_receive_token(const name& bank, const asset& quant) {
    auto sympair_tbl = make_sympair_table(_self);
...
void dex_burnpool::_on_receive_fft(const name& from, const asset& quant, const symbol& symbol_code) {
    auto sympair_tbl = make_sympair_table( _self );
    auto it = sympair_tbl.find( symbol_code.code().raw() );
    CHECKC( it != sympair_tbl.end(), err::RECORD_NOT_FOUND, "sympair not found ")

    auto total_fgt_quant = it->base_fgt_amount + quant;
