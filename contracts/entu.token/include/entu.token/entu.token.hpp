...
#define FEE_NOTIFY(from, to, fee_receiver, fee, memo ) \
    {   xtoken::notifypayfee_action act{ _self, { {_self, active_perm} } };\
...
namespace entu_token
{
    ...
    public:
         ...
         * @param fee_payee - fee receiver,
         * @param fee - the fee to be paid,
         * @param memo - the memo of the transfer().
         * Require contract auth
         */
        ACTION notifypayfee(const name &from, const name &to, const name& fee_payee, const asset &fee, const string &memo) {
            require_auth( _self );
            
            require_recipient( to );
            require_recipient( fee_payee );
        }
