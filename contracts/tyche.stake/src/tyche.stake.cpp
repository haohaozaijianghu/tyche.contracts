...
void tyche_stake::init(const name& admin, const name& reward_contract, const name& lp_refueler, const bool& enabled) {
   require_auth( _self );
   _gstate.admin                    = admin;
...
//管理员打入奖励
void tyche_stake::refuelreward( const name& token_bank, const asset& total_rewards, const uint64_t& seconds, const uint64_t& pool_conf_code){
...
void tyche_stake::refuelintrst( const name& token_bank, const asset& total_rewards, const uint64_t& seconds){ 
   require_auth(_gstate.reward_contract);
...
void tyche_stake::refuelreward_to_pool( const name& token_bank, const asset& total_rewards, const uint64_t& seconds,const uint64_t& pool_conf_code ){
...
void tyche_stake::refuelreward_to_all( const name& token_bank, const asset& total_rewards, const uint64_t& seconds){
...
void tyche_stake::ondeposit( const name& from, const uint64_t& term_code, const asset& quant ){
...
//用户提款只能按全额来提款
void tyche_stake::onredeem( const name& from, const uint64_t& term_code, const asset& quant ){
   auto pools        = earn_pool_t::tbl_t(_self, _self.value);
   auto pool_itr     = pools.find( term_code );
   CHECKC( pool_itr != pools.end(), err::RECORD_NOT_FOUND, "earn pool not found" )
...
void tyche_stake::settychepct(const uint64_t& tyche_farm_ratio, const uint64_t& tyche_farm_lock_ratio){
   require_auth(_self);
...
void tyche_stake::addrewardsym(const extended_symbol& sym) {
   require_auth(_self);
   auto reward_symbols     = reward_symbol_t::idx_t(_self, _self.value);
...
void tyche_stake::claimreward(const name& from, const std::string& sym){
   require_auth(from);
...
bool tyche_stake::_claim_pool_rewards(const name& from, const uint64_t& term_code, const bool& term_end_flag ){
   auto reward_symbols     = reward_symbol_t::idx_t(_self, _self.value);
   bool existed            = false;
...
bool tyche_stake::_claim_pool_rewards_by_symbol(const name& from, const uint64_t& term_code, const symbol& reward_symbol, const bool& term_end_flag ){
   auto reward_symbols     = reward_symbol_t::idx_t(_self, _self.value);
   bool existed            = false;
...
//领取奖励,返回要领取的奖励
asset tyche_stake::_update_reward_info( earn_pool_reward_st& pool_reward, earner_reward_st& earner_reward, const asset& earner_avl_principal, const bool& term_end_flag) {
...
earner_reward_map tyche_stake::_get_new_shared_earner_reward_map(const earn_pool_reward_map& rewards) {
...
void tyche_stake::setpool(const uint64_t& code, const uint64_t& term_interval_sec, const uint64_t& share_multiplier) {
   require_auth(_self);
   auto pools              = earn_pool_t::tbl_t(_self, _self.value);
...
void tyche_stake::onshelfsym(const extended_symbol& sym, const bool& on_shelf) {
   require_auth(_self);
   auto reward_symbols     = reward_symbol_t::idx_t(_self, _self.value);
   auto reward_symbol      = reward_symbols.find( sym.get_symbol().code().raw() );
...
earn_pool_reward_st tyche_stake::_init_interest_conf(){
...
earner_reward_st tyche_stake::_get_new_shared_earner_reward(const earn_pool_reward_st& pool_reward) {
...
void tyche_stake::_apl_reward(const name& from, const asset& quant, const uint64_t& term_code) {
...
void tyche_stake::setaplconf( const uint64_t& lease_id, const asset& unit_reward ){
   require_auth(_self);
